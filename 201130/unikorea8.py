...
class Unikorea8Spider(scrapy.Spider):
    name = 'unikorea8'
    allowed_domains = ['https://www.unikorea.go.kr']
    start_urls = ['https://www.unikorea.go.kr/books/archive/archive/']

    def __init__(self):
        scrapy.Spider.__init__(self)
        self.start_urls = 'https://www.unikorea.go.kr/books/archive/archive/'
        # 몽고에 넣겠
    ...
    def parse(self, response):
        ...
        while True:
            if page_no > last_page_no:
                break
            link = "https://www.unikorea.go.kr/books/archive/archive/?mode=list&boardId=bbs_0000000000000043&searchCondition=all&searchKeyword=&pageIdx="\
                   + str(page_no) + "&searchKeyword-ori="
    ...
    def parse_each_pages(self, response):
        ...
        # print(last_page_no)
        last = response.xpath('//*[@id="bbsForm"]/table/tbody/tr[1]/td[1]/text()').get()
        if page_no == last_page_no:
            category_last_no = int(last)
        else:
            first = response.xpath('//*[@id="bbsForm"]/table/tbody/tr[15]/td[1]/text()').get()
        ...
        while True:
            ...
            url = response.xpath('//*[@id="bbsForm"]/table/tbody/tr[' + str(category_no) + ']/td[2]/a/@href').get()
            url = "https://www.unikorea.go.kr/books/archive/archive/" + url
    ...
    def parse_post(self, response):
        ...
        title = response.xpath('//*[@id="aticleTitle"]/text()').get()
        print(title)
        ...
        date = response.xpath('//*[@id="bbsForm"]/div/article/div[1]/div/dl[2]/dd/text()').get()
