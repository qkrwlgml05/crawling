...
class Boardbotkinu38Spider(scrapy.Spider):
    ...
    def parse(self, response):
        page_no = 1
        last_page_text = response.xpath('//*[@id="cmsContent"]/div[1]/div[1]/text()[2]').extract()
    ...
    def parse_each_pages(self, response):
        ...
        last = response.xpath('//*[@id="cmsContent"]/div[2]/table/tbody/tr[1]/td[1]/text()').get()
        if page_no == last_page_no:
            category_last_no = int(last)
        else:
            first = response.xpath('//*[@id="cmsContent"]/div[2]/table/tbody/tr[10]/td[1]/text()').get()
            category_last_no = int(last) - int(first) + 1
        category_no = 1
        while True:
            if(category_no > category_last_no):
                break
            category_link = response.xpath('//*[@id="cmsContent"]/div[2]/table/tbody/tr['+ str(category_no) +']/td[2]/a/@href').get()
            url = 'http://www.kinu.or.kr' + category_link
            print(url)
            #number = response.xpath('//*[@id="cmsContent"]/div[2]/table/tbody/tr['+str(category_no)+']/td[1]').get()
            #print(number)
            item = CrawlnkdbItem()
            date = response.xpath('//*[@id="cmsContent"]/div[2]/table/tbody/tr['+str(category_no)+']/td[3]').xpath('string()').get()
            item[config['VARS']['VAR4']] = date
            yield scrapy.Request(url, callback=self.parse_post, meta={'item':item}, dont_filter=True)
            category_no += 1

    def parse_post(self, response):
        print("parse_post")
        title = response.xpath('//*[@id="cmsContent"]/div[1]/table/thead/tr[1]/td').xpath('string()').get()
        body = response.css('#tab_con > div').xpath('string()').get()
        print(body)

        writer = response.xpath('//*[@id="cmsContent"]/div[1]/table/tbody/tr[2]/td').xpath('string()').get()
        
        top_category = response.xpath('//*[@id="container"]/div[3]/div[1]/div/h2').xpath('string()').get()
        ...
        file_name = title
        file_icon = response.xpath('//*[@id="cmsContent"]/div[1]/table/tbody/tr[4]/td/a/img').get()
        if file_icon:
            file_download_url = 'http://www.kinu.or.kr'+response.xpath('//*[@id="cmsContent"]/div[1]/table/tbody/tr[4]/td/a/@href').extract()[0]
