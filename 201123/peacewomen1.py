...
class Peacewomen1Spider(scrapy.Spider):
    name = 'peacewomen1'
    allowed_domains = ['http://www.peacewomen.or.kr']
    start_urls = ['http://www.peacewomen.or.kr/index.php?mid=wmp_pds_women']

    def __init__(self):
        scrapy.Spider.__init__(self)
        self.start_urls = 'http://www.peacewomen.or.kr/index.php?mid=wmp_pds_women'
        # 몽고에 넣겠다
    ...
    def parse(self, response):
        # print(response)
        # 페이지 개수
        total_page_text = response.xpath('//*[@id="s_mid21_wrap0"]/div/div[2]/div[1]/a[2]/@href').extract()
        ...
        while True:
            if page_no > last_page_no:
                break
            link = "http://www.peacewomen.or.kr/index.php?mid=wmp_pds_women&page=" + str(page_no)
    ...
    def parse_each_pages(self, response):
        ...
        #print(last_page_no)
        last = response.xpath('//*[@id="board_list"]/table/tbody/tr[1]/td[1]/text()').get()
        last = 15
        if page_no == last_page_no:
            category_last_no = int(last)
        else:
            first = response.xpath('//*[@id="board_list"]/table/tbody/tr[15]/td[1]/text()').get()
        ...
        while True:
            ...
            url = "http://www.peacewomen.or.kr/index.php?mid=wmp_pds_women&page=" + url
            #print(url)
    ...
    def parse_post(self, response):
        ...
        top_category = response.xpath('//*[@id="s_left11_wrap_st2"]/ul/li[1]/a/text()').get()

        item['post_title'] = title.strip()
        item['post_date'] = date.strip()
        item['post_writer'] = writer.strip()
        item['post_body'] = body_text.strip()
        item['published_institution'] = "평화를만드는여성회"
        item['published_institution_url'] = "http://www.peacewomen.or.kr/index.php?mid=wmp_pds_women&page="
        ...
        if file_icon:
            ...
            if file_icon.find("hwp") != -1:
                print('find hwp')
                yield scrapy.Request(file_download_url, callback=self.save_file_hwp, dont_filter=True, meta={'item': item})  #
            else:
